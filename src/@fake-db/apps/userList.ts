...
const data: { users: UsersType[] } = {
  users: [
    ...
    // {
    //   id: 24,
    //   nickname: 'Cleavland Hatherleigh',
...
mock.onPost('/apps/users/add-user').reply((config) => {
  ...
  let lastIndex = 0;
  if (length) {
    lastIndex = Number(data.users[length - 1].id);
  }
  user.id = lastIndex + 1;
