...
const defaultColumns = [
  ...
  // },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography variant="body2">{moment(row.regDate).format('YYYY-MM-DD HH:mm')}</Typography>
    ),
  },
];

// permission data 불러오는 메소드
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }: CellType) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => handleEditPermission(row.menuName)}>
            <PencilOutline />
          </IconButton>
          <IconButton>
            <DeleteOutline />
      ...
      <Grid container spacing={6}>
        ...
        <Grid item xs={12}>
          <Card>
            ...
            <DataGrid
              autoHeight
              getRowId={(row: any) => row.permissionId}
              rows={store.data}
              columns={columns}
              pageSize={pageSize}
